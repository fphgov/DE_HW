...
- [Poetry](https://python-poetry.org/docs/#installation) (for local development and adding packages)
...
Once running, open: **http://localhost:8081**
...
```bash
# 1. Add the package via Poetry (updates pyproject.toml and poetry.lock)
...
**Why the script?** `requirements.txt` is generated by `pip-compile` using the Airflow constraint file. This guarantees your new dependency is compatible with Airflow's own pinned versions — if it isn't, the script will fail with a clear error before anything breaks.
...
```bash
cd packages/template_package
poetry install          # install dev deps locally
poetry run pytest
```
...
`requirements.txt` is **not** a plain pip requirements file and is **not** generated by `poetry export`. It is produced by `pip-compile` against the [Airflow 2.10.4 constraints file](https://raw.githubusercontent.com/apache/airflow/constraints-2.10.4/constraints-3.12.txt).
